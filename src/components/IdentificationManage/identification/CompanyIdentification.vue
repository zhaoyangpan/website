	  	...
	  	<div class="main-card-top">
			  ...
			  <el-col :span="20" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="企业认证信息"></el-tab-pane>					  			  
					</el-tabs>
			  	</div>
			  </el-col>	
			  ...
			  <el-col :span="6">
			  	<div class="grid-content bg-purple">
			  		<el-form-item label="企业名称">				        
					    <el-input type="text·" class="ident-input" :disabled='update' v-model="identificationForm.comname"></el-input>
					</el-form-item>
			  		<el-form-item label="法人姓名">
				        <el-input type="text" class="ident-input" :disabled='update' v-model="identificationForm.legalperson"></el-input>
				   </el-form-item>				    
				    ...
				    <el-form-item label="营业执照号码">				        
				        <el-input type="text" class="ident-input" :disabled='update' v-model="identificationForm.blicence"></el-input>
				    </el-form-item>
       ...
       methods: {
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.CompanyApplyUrl(para).then(res => {
       	 	 	console.log(res)
				...
				this.fileList4[0].url = res.data.data.blicence_img;
				this.identificationForm = {
		          comname: res.data.data.comname,
		          legalperson: res.data.data.legalperson,
		          mphone: res.data.data.mphone,
		          phone: res.data.data.phone,
		          blicence: res.data.data.blicence,
			...
			.catch(err => {
			  this.$message.error('错了哦，获取企业用户认证信息失败');
			});
       	 },
       	 //通过认证
       	 CrossIdent:function() {
	           ...
	           blicenceImg:this.fileList4[0].url,
	           ...
	           comname:this.identificationForm.comname,
	           blicence:this.identificationForm.blicence,
	           legalperson:this.identificationForm.legalperson
	        }
	        this.$confirm('确认认证吗?', '提示', {
			}).then(() => {
			let para = Object.assign({}, this.crossIdentification);	
			api.CompanyCrossApply(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '认证成功',
					type: 'success'
				});		
				this.addMessage('企业认证反馈','企业认证已通过审核')
	     ...
	     RejectIdent:function(){
			...
			api.CompanyRejectApply(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '已驳回',
					type: 'success'
				});	
				this.addMessage('企业认证反馈',this.reson)
		    ...
		    saveinfo(){
			   let routerParams = this.$route.query.id
				...
				let para = Object.assign({}, this.crossIdentification);	
				api.CompanyUpdateUrl(para).then((res) => {
					if(res.data.code == 200){
					    this.$message({
						message: '保存用户信息成功',
						type: 'success'
