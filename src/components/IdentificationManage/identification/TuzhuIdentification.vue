	  	...
	  	<div class="main-card-top">
			  ...
			  <el-col :span="20" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="土著认证信息"></el-tab-pane>					  			  
					</el-tabs>
			  	</div>
			  </el-col>	
       ...
       methods: {
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.TuzhuApplyUrl(para).then(res => {
       	 	 	console.log(res)
			...
			.catch(err => {
			  this.$message.error('错了哦，获取土著用户认证信息失败');
			});
       	 },
       	 //通过认证
       	 CrossIdent:function() {
			...
			api.TuzhuCrossApply(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '认证成功',
					type: 'success'
				});		
				this.addMessage('土著认证反馈','土著认证已通过审核')
				this.$router.push({path:'/TuzhuManage'});
				}else{
					this.$message({
	     ...
	     RejectIdent:function(){
			...
			api.TuzhuRejectApply(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '已驳回',
					type: 'success'
				});	
				this.addMessage('土著认证反馈',this.reson)
		    ...
		    saveinfo(){
			   let routerParams = this.$route.query.id
				...
				let para = Object.assign({}, this.crossIdentification);	
				api.TuzhuUpdateUrl(para).then((res) => {
					if(res.data.code == 200){
					    this.$message({
						message: '保存用户信息成功',
						type: 'success'
