	  ...
	  <el-main class="main-card">
	  	<div class="main-card-top">
			  ...
			  <el-col :span="4" class="search-col18">
			  	<div class="grid-content bg-purple">
			  		 <el-button type="primary"  @click="EditUpdate">编辑政府信息</el-button>
			  	</div>
			  </el-col>
		    </el-row>
	  	</div>
	  	<div class="main-card-bottom">
	  	  <el-form ref="form" :model="identificationForm" label-width="80px">
			  ...
			  <el-col :span="6">
			  	<div class="grid-content bg-purple">
			  		<el-form-item label="单位名称">				        
					    <el-input type="text" class="ident-input" :disabled='update' v-model="identificationForm.orgname"></el-input>
					</el-form-item>
			  		<el-form-item label="行政级别">
				        <el-input type="text" class="ident-input" :disabled='update' v-model="identificationForm.pcclevel_id"></el-input>
				   </el-form-item>				    
					<el-form-item label="负责人">				       
				        <el-input type="text" class="ident-input" :disabled='update' v-model="identificationForm.mainperson"></el-input>
				   </el-form-item>
				    ...
				    <el-form-item label="单位座机">				       
				        <el-input type="text" class="ident-input" :disabled='update' v-model="identificationForm.phone"></el-input>
				    </el-form-item>
			  ...
			  <el-col :span="9">
			  	<div class="grid-content bg-purple-light">
			  		<el-form-item label="单位证明照片" class="idcard">				       
						<el-upload
						  :class="{disabled:type == true}"	
						  :action="importFileUrl"
						  list-type="picture-card"
				  ...
				  <el-form-item label="政府头像" class="idcard">				       
						<el-upload
						  :class="{disabled:type == true}"	
						  :action="importFileUrl"
						  list-type="picture-card"
       ...
       methods: {
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.GovernmentApplyUrl(para).then(res => {
       	 	 	console.log(res)
				...
				this.fileList4[0].url = res.data.data.prove_img;
				this.identificationForm = {
		          orgname: res.data.data.orgname,
		          pcclevel_id: res.data.data.pcclevel_id,
		          mainperson: res.data.data.mainperson,
			...
			.catch(err => {
			  this.$message.error('错了哦，获取政府用户认证信息失败');
			});
       	 },
       	 //通过认证
       	 CrossIdent:function() {
	           ...
	           phone:this.identificationForm.phone,
	           orgname:this.identificationForm.orgname,
	           mainperson:this.identificationForm.mainperson,
	           pcclevelId:this.identificationForm.pcclevel_id
	        }
	        this.$confirm('确认认证吗?', '提示', {
			}).then(() => {
			let para = Object.assign({}, this.crossIdentification);	
			api.GovernmentCrossApply(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '认证成功',
					type: 'success'
				});		
				this.addMessage('政府认证反馈','政府认证已通过审核')
				this.$router.push({path:'/GovernmentManage'});
				}else{
					this.$message({
	     ...
	     RejectIdent:function(){
			...
			api.GovernmentRejectApply(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '已驳回',
					type: 'success'
				});
				this.addMessage('政府认证反馈',this.reson)
		    ...
		    saveinfo(){
			   let routerParams = this.$route.query.id
				...
				let para = Object.assign({}, this.crossIdentification);	
				api.GovernmentUpdateUrl(para).then((res) => {
					if(res.data.code == 200){
					    this.$message({
						message: '保存用户信息成功',
						type: 'success'
