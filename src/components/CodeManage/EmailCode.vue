	...
	<el-container>	  
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="code" label="验证码" show-overflow-tooltip> </el-table-column>
			  	...
			  	<div class="grid-content bg-purple-dark">
			  		<el-form :model="addLabel" :label-position="labelPosition" label-width="80px" :rules="addLabelRules" ref="addLabel">
						<el-form-item label="邮箱号" prop="email">
							<el-input v-model="addLabel.email" auto-complete="off"></el-input>
						</el-form-item>
			...
			<el-row>
		     <el-col :span="16">
				...
				<el-form :model="editLabel" :label-position="labelPosition" label-width="80px" :rules="editLabelRules" ref="editLabel">
					<el-form-item label="邮箱号" prop="email">
						<el-input v-model="editLabel.email" auto-complete="off"></el-input>
					</el-form-item>
					<el-form-item label="验证码" prop="code">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
      		api.GetEmailCodeList(para).then(res => {	
				console.log(res)
				  let _this = this;
				  _this.totalCount = res.data.data.totalRow;			  
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            var obj = {}
		            obj.email = res.data.data.list[i].email;
       	  ...
       	  addLabelSubmit:function  () {
				...
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						console.log(para)
						api.AddEmailCode(para).then((res) => {
							console.log(res)
							this.$message({
								message: '添加手机验证码成功',
								type: 'success'
...
//			this.editLabel = Object.assign({}, row);
	      	...
	      	this.$refs.editLabel.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							para.expire_time = (!para.expire_time || para.expire_time == '') ? '' : util.formatDate.format(new Date(para.expire_time), 'yyyy-MM-dd hh:mm:ss');							
								api.EditEmailCode(para).then((res) => {
		  ...
		  handleDel: function (index, row) {
			...
			this.LabellistLoading = true;
			let para = { id: row.id };
			api.DeleteEmailCode(para).then((res) => {
				    this.LabellistLoading = false;
					this.$message({
						message: '删除成功',
						type: 'success'
		  ...
		  batchRemove: function () {
			...
			this.$confirm('确认删除选中记录吗？', '提示', {
				...
				api.BatchDeleteEmailCode({'ids':ids}).then((res) => {
					this.$message({
						message: '删除成功',
						type: 'success'
					});
