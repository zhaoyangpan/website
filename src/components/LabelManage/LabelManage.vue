	...
	<el-container>	  
	  <el-main class="main-card">
	  	<div class="main-card-top">
			  ...
			  <el-col :span="15" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="系统标签" name="1"></el-tab-pane>
					  <el-tab-pane label="自定义标签" name="2"></el-tab-pane>					  
					</el-tabs>
			  	</div>
			  </el-col>
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="tname" label="标签名称" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="post_count" label="帖子数量"> </el-table-column>
		    <el-table-column prop="baike_count" label="百科数量"> </el-table-column>
		    <el-table-column prop="topic_count" label="关注数量"> </el-table-column>
		    <el-table-column prop="tagsys_name" label="标签类型" show-overflow-tooltip> </el-table-column>		    
		    ...
		    <el-table-column prop="verify_id" label="审核状态" width="200"> 
		    ...
		    <el-table-column prop="ishot" label="热门"> 
		        <template slot-scope="scope">	
		    ...
		    <el-table-column label="操作" width="350" align='center' fixed="right" show-overflow-tooltip> 
		       <template slot-scope="scope" >
		       	    <el-button type="danger" size="small" @click="LabelUp(scope.$index, scope.row)">升级</el-button>
		       	    <router-link :to="{path: '/PostManage',query:{id: scope.row.id}}">
		       	    	<el-button  size="small" >帖子</el-button>
		       	    </router-link>
					<el-button  size="small" @click="">百科</el-button>
		  ...
		  <!--新增标签-->
		  <el-dialog title="新增标签" :visible.sync="addLabelVisible" width="40%" :close-on-click-modal="false">
		  	<el-row>
			  <el-col :span="16">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
	        api.GetLabelList(para).then(res => {	
				console.log(res)
				  let _this = this;
				  ...
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            ...
		            obj.tagsys_name = res.data.data.list[i].tagsys_name
       	  ...
       	  addLabelSubmit:function  () {
				...
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						api.AddLabelInfo(para).then((res) => {
							console.log(res)
							this.$message({
								message: '添加标签成功',
								type: 'success'
					...
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							//para.updated_time = (!para.updated_time || para.updated_time == '') ? '' : util.formatDate.format(new Date(para.updated_time), 'yyyy-MM-dd hh:mm:ss');							
								api.EditLabelInfo(para).then((res) => {
		  ...
		  batchRemove: function () {
			...
			this.$confirm('确认删除选中记录吗？', '提示', {
				...
				api.BatchDeleteLabel({'ids':ids}).then((res) => {
					this.$message({
						message: '删除成功',
						type: 'success'
					});
	      ...
	      LabelUp(index, row){
	      	let para = {
	      		id:row.id,
	      		tagtype_id:row.tagtype_id == 1 ? 2 : 1,
	      		tname:row.tname,
	      		verify_id:1,
	      	}
	      	console.log(row)
	      	this.$confirm('确认升级吗, 是否继续?', '提示', {
