	...
	<el-container>
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="title" label="百科名称"> </el-table-column>
		    <el-table-column prop="nickname" label="发表人"> </el-table-column>
		    <el-table-column prop="created_time" label="发表日期"> </el-table-column>
		    ...
		    <el-table-column label="操作" width="100" align='center' fixed="right" show-overflow-tooltip> 
		       <template slot-scope="scope" >
					<router-link :to="{path: '/BaikeAudit',query:{id: scope.row.checkId}}">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
	        api.GetBaikeAuditList(para).then(res => {
	        	  console.log(res)
				  let _this = this;
				  ...
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            ...
		            obj.update_reson = res.data.data.list[i].update_reson
		            obj.verify_id = res.data.data.list[i].verify_id	
		            obj.checkId = res.data.data.list[i].checkId
		            obj.baikeId = res.data.data.list[i].baikeId
		            data[i] = obj
		       }				  			
