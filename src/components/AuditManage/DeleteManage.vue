	...
	<el-container>
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="baike_name" label="百科名称" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="nickname" label="提交人" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="describe" label="删除申请描述" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="problem_types" label="类型" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="result" label="处理结果" show-overflow-tooltip> </el-table-column>		    
		    <el-table-column prop="created_time" label="发表日期" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="verify_id" label="认证状态" show-overflow-tooltip> 
		     <template slot-scope="scope">	
		      <el-tag :type="articleStatusTag(scope.row.verify_id)" close-transition>
                {{scope.row.verify_id == 1 ? "已审核":((scope.row.verify_id == 2) ? "已驳回":"待审核")}}
              </el-tag>
             </template>
		    </el-table-column>		  
		    <el-table-column label="操作" width="100" align='center' fixed="right" show-overflow-tooltip> 
		       <template slot-scope="scope" >
					<router-link :to="{path: '/DeleteAudit',query:{id: scope.row.checkId}}">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
	        api.GetDeleteAuditList(para).then(res => {
	        	  console.log(res)
				  let _this = this;
				  _this.totalCount = res.data.data.totalRow;			  
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            var obj = {}
		            obj.baike_name = res.data.data.list[i].baike_name;
		            obj.nickname = res.data.data.list[i].nickname;
		            obj.created_time = res.data.data.list[i].created_time
		            obj.problem_types = res.data.data.list[i].problem_types
		            obj.verify_id = res.data.data.list[i].verify_id	
		            obj.result = res.data.data.list[i].result
		            obj.describe = res.data.data.list[i].describe
		            obj.checkId = res.data.data.list[i].checkId
		            data[i] = obj
		       }				  			
