	  ...
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column label="操作" width="100" align='center' fixed="right" show-overflow-tooltip> 
		       <template slot-scope="scope" >
					<router-link :to="{path: '/ServiceAudit',query:{id: scope.row.id}}">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
	        api.GetServiceAuditList(para).then(res => {
	        	  console.log(res)
				  let _this = this;
				  _this.totalCount = res.data.data.totalRow;			  
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            var obj = {}
		            obj.service_name = res.data.data.list[i].service_name;
		            ...
		            obj.store_id = res.data.data.list[i].store_id
		            data[i] = obj
		       }				  			
