	...
	<el-container>
	  <el-main class="main-card">
	  	<div class="main-card-top">
			  ...
			  <el-col :span="20" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="百科修改审核"></el-tab-pane>					  			  
					</el-tabs>
			  	</div>
			  </el-col>	
	  	...
	  	<div class="main-card-bottom">
			...
			<el-dialog :title="baikeTitle" :visible.sync="DetailDialogVisible" width="70%" center>
				  <div class="baike_change_wrap">
				  	<div class="baike_change_left">
				  		<h3>修改前</h3>
				  		<div class="baike_change_left_content" v-html="biakeList.before"></div>
				  	</div>
				  	<div class="baike_change_right">
				  		<h3>修改后</h3>
				  		<div class="baike_change_right_content" v-html="biakeList.later"></div>
				  	</div>
				  </div>				  
				  <span slot="footer" class="dialog-footer">
	...
	export default {
       ...
       methods: {
       	//获取单条信息
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.GetChangeAuditInfo(para).then(res => {		
       	 	 	console.log(res)
       	 ...
       	 checkDetail(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.CheckeChangeDetail(para).then(res => {		
       	 	 	console.log(res)
			...
			api.CrossChangeAudit(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '审核成功',
					type: 'success'
				});		
				this.addMessage('百科修改审核反馈','你提交的 "' +this.identificationForm.title+ '" 已通过审核')
				this.$router.push({path:'/ChangeManage'});
				}else{
					this.$message({
	     ...
	     RejectIdent:function(){
			...
			api.RejectChangeAudit(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '已驳回',
					type: 'success'
				});	
				this.addMessage('百科修改审核反馈',this.return_reson)
...
.disabled .el-upload--picture-card{
 ...
 .time {
  ...
  .baike_change_left>h3,.baike_change_right>h3{
  	text-align: center;
  }
  .baike_change_left_content,.baike_change_right_content{
  	padding: 0px 20px;
  }
</style>