	...
	<el-container>
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="store_name" label="店铺名称" show-overflow-tooltip> </el-table-column>
		    ...
		    <el-table-column prop="store_address" label="店铺地址" show-overflow-tooltip> </el-table-column>		    		    
		    ...
		    <el-table-column label="操作" width="350" align='center' fixed="right" show-overflow-tooltip> 
		       <template slot-scope="scope" >
					<router-link :to="{path: '/ShopReport',query:{id: scope.row.id}}">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
	        api.GetShopReportList(para).then(res => {
	        	  console.log(res)
				  let _this = this;
				  ...
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            ...
		            obj.store_name = res.data.data.list[i].store_name		          
		            obj.user_id = res.data.data.list[i].user_id	
		            obj.serviceline = res.data.data.list[i].serviceline
		            obj.store_address = res.data.data.list[i].store_address
		            obj.store_keeper = res.data.data.list[i].store_keeper
		            data[i] = obj
		       }				  			
