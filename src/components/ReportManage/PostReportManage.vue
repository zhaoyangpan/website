	...
	<el-container>
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="issuer" label="发布人" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="publish_time" label="发布日期" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="nickname" label="举报人" show-overflow-tooltip> </el-table-column>
		    <el-table-column prop="created_time" label="提交日期" show-overflow-tooltip> </el-table-column>
		    ...
		    <el-table-column label="操作" width="350" align='center' fixed="right" show-overflow-tooltip> 
		       <template slot-scope="scope" >
					<router-link :to="{path: '/PostReport',query:{id: scope.row.id}}">
	...
	export default {
       ...
       methods: {
			...
			this.LabellistLoading = true;	
      		let data = [];
	        api.GetPostReportList(para).then(res => {
	        	  console.log(res)
				  let _this = this;
				  ...
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            ...
		            obj.issuer = res.data.data.list[i].issuer
		            obj.publish_time = res.data.data.list[i].publish_time
		            data[i] = obj
		       }				  			
