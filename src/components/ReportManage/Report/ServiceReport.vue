	  	...
	  	<div class="main-card-top">
			  ...
			  <el-col :span="20" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="服务举报信息"></el-tab-pane>					  			  
					</el-tabs>
			  	</div>
			  </el-col>	
	...
	export default {
       ...
       methods: {
       	//获取单条信息
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.GetServiceInfo(para).then(res => {
       	 	 	console.log(res)				
			...
			api.CrossServiceReport(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '举报成功',
					type: 'success'
				});		
				this.addMessage('服务举报反馈','您提交的服务 "' +this.identificationForm.service_name+ '" 已处理')
				this.$router.push({path:'/ServiceReportManage'});
				}else{
					this.$message({
	     ...
	     RejectIdent:function(){
			...
			api.RejectServiceReport(para).then((res) => {
				console.log(res)
				if(res.data.code == 200){
				    this.$message({
					message: '已驳回',
				...
				this.addMessage('服务举报反馈',this.return_reson)
