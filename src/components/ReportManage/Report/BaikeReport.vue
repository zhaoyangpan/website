	  	...
	  	<div class="main-card-top">
			  ...
			  <el-col :span="20" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="百科举报信息"></el-tab-pane>					  			  
					</el-tabs>
			  	</div>
			  </el-col>	
	...
	export default {
       ...
       methods: {
       	//获取单条信息
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.GetBaikeInfo(para).then(res => {
       	 	 	console.log(res)				
			...
			api.CrossBaikeReport(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '举报成功',
					type: 'success'
				});	
				this.addMessage('百科举报反馈','您提交的 "' +this.identificationForm.title+ '" 已处理')
				this.$router.push({path:'/BaikeReportManage'});
				}else{
					this.$message({
	     ...
	     RejectIdent:function(){
			...
			api.RejectBaikeReport(para).then((res) => {
				console.log(res)
				if(res.data.code == 200){
				    this.$message({
					message: '已驳回',
				...
				this.addMessage('百科举报反馈',this.return_reson)
