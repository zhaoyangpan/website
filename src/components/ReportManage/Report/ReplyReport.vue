	  	...
	  	<div class="main-card-top">
			  ...
			  <el-col :span="20" class="search-col18">
			  	<div class="grid-content bg-purple">
					  ...
					  <el-tab-pane label="回复举报信息"></el-tab-pane>					  			  
					</el-tabs>
			  	</div>
			  </el-col>	
			  ...
			  <el-col :span="8">
			  	<div class="grid-content bg-purple">
			  		<el-form-item label="回复内容">				        
					    <el-input type="textarea" class="ident-input" :disabled='update' v-model="identificationForm.comment_content"></el-input>
					</el-form-item>			  		
	...
	export default {
       ...
       methods: {
       	//获取单条信息
       	 GetIdentificationInfo:function(){
       	 	...
       	 	let para = {
       	 	 ...
       	 	 api.GetReplyInfo(para).then(res => {
       	 	 	console.log(res)				
			...
			api.CrossReplyReport(para).then((res) => {
				if(res.data.code == 200){
				    this.$message({
					message: '举报成功',
					type: 'success'
				});		
				this.addMessage('回复举报反馈','您提交的回复 "' +this.identificationForm.comment_content+ '" 已处理')
				this.$router.push({path:'/ReplyReportManage'});
				}else{
					this.$message({
	     ...
	     RejectIdent:function(){
			...
			api.RejectReplyReport(para).then((res) => {
				console.log(res)
				if(res.data.code == 200){
				    this.$message({
						message: '已驳回',
				...
				this.addMessage('回复举报反馈',this.return_reson)
