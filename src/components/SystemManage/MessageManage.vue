	...
	<el-container>
	  <el-main class="main-card">
	  	...
	  	<div class="main-card-bottom">
		    ...
		    <el-table-column prop="messagetpl_title" label="模板标题"  show-overflow-tooltip> </el-table-column>
			  ...
			  <el-col :span="16">
			  	<div class="grid-content bg-purple-dark">
					<el-form :model="addAdvert" label-width="80px" :rules="addAdvertRules" ref="addAdvert">
						...
						<el-form-item label="消息模板" prop="messagetpl_id">
							<el-select v-model="addAdvert.messagetpl_id" placeholder="请选择" style="width: 100%;">
			  ...
			  <el-col :span="16">
			  	<div class="grid-content bg-purple-dark">
					<el-form :model="editAdvert" label-width="80px" :rules="editAdvertRules" ref="editAdvert">
						...
						<el-form-item label="消息模板" prop="messagetpl_id">
							<el-select v-model="editAdvert.messagetpl_id" placeholder="请选择" style="width: 100%;">
							    <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"> </el-option>
							</el-select>
						</el-form-item>										
	...
	export default {
       ...
       methods: {
			...
			this.advertlistLoading = true;	
      		let data = [];
	        api.GetMessageList(para).then(res => {	
				console.log(res)
				  let _this = this;
				  ...
				  for (let i = 0; i < res.data.data.list.length; i++) {              
		            ...
		            obj.messagetpl_id = res.data.data.list[i].messagetpl_id
		            obj.messagetpl_title = res.data.data.list[i].messagetpl_title
		            obj.receiver_id = res.data.data.list[i].receiver_id
		            obj.send_time = res.data.data.list[i].send_time
		            obj.sender_id = res.data.data.list[i].sender_id
		            obj.sequence = res.data.data.list[i].sequence		           
		            obj.status_id = res.data.data.list[i].status_id	
		            obj.status_name = res.data.data.list[i].status_name
			...
			.catch(err => {
       	  ...
       	  getAdverposition:function(){
       	  	let data = [];
	      	api.GetAllMessagetplList().then((res) => {
				for (let i = 0; i < res.data.data.length; i++) {              
		            let obj = {}
		            obj.value = res.data.data[i].id;
		            obj.label = res.data.data[i].title;
		            data[i] = obj;
		        }
				this.options = data;
			});
...
//			this.editPowerForm = Object.assign({}, row);
            ...
            this.editAdvert = {
	           title:row.title,
	           messagetpl_id:row.messagetpl_id,
		  ...
		  handleDel: function (index, row) {
			...
			let para = { id: row.id };
			console.log(para)
			api.DeleteMessage(para).then((res) => {
		  ...
		  batchRemove: function () {
			...
			this.$confirm('确认删除选中记录吗？', '提示', {
				...
				api.BatchDeleteMessage({'ids':ids}).then((res) => {
					this.$message({
						message: '删除成功',
						type: 'success'
					});
